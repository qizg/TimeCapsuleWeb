package com.sinwn.capsule.constant;

public class StrConstant {
    public static final String SUCCESS = "SUCCESS";

    public static final String PASSWORD_SALT = "sinwn";

    public static final String SYSTEM_ERROR = "系统错误，请稍后再试";

    public static final String NO_USER = "用户不存在，请检查您的输入账号";

    public static final String NO_DATA = "暂无数据";

    public static final String LOGIN_ERROR = "用户名或密码错误";

    public static final String AUTHORIZED_ERROR = "暂无权限访问";

    public static final String REQ_ERROR = "请求参数有误";

    public static final String SING_UP_HAS_ERROR = "用户已注册，请直接登录";
}
